<?
	...
	// Admins are allowed to see the full media directory
	$memberdir=$member->id;
	if ($member->admin){
		$memberdir='';
	} 
	
	// create member media dir if necessary (adapted from media.php Nucleus version 3.2)
	if (!@is_dir($DIR_MEDIA.$memberdir)) {
		$oldumask = umask(0000);
		if (!@mkdir($DIR_MEDIA.$memberdir, 0777))
			echo _ERROR_BADPERMISSIONS;
		umask($oldumask);                
	}
...
*/
$IMConfig['base_dir'] = $DIR_MEDIA.$memberdir;
...
*/
$IMConfig['base_url'] = $CONF['MediaURL'].$memberdir;
