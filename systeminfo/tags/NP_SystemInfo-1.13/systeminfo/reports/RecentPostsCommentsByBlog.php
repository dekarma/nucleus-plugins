<?php
	...
	*/
	...
	while ($row = mysql_fetch_assoc($res)) {
		...
		$data[] = intval($row['bid']);
		$data[] = $row['name'];
		$num = intval(quickQuery("SELECT COUNT(*) as result FROM ".sql_table('item')." WHERE iblog=".intval($row['bid'])." AND UNIX_TIMESTAMP(itime) > UNIX_TIMESTAMP() - 86400 "));
		$tot1 = $tot1 + $num;
		$data[] = $num;
		...
		$tot1c = $tot1c + $num;
		$data[] = $num;		
		$num = intval(quickQuery("SELECT COUNT(*) as result FROM ".sql_table('item')." WHERE iblog=".intval($row['bid'])." AND UNIX_TIMESTAMP(itime) > UNIX_TIMESTAMP() - 604800 "));
		$tot7 = $tot7 + $num;
		$data[] = $num;		
		...
		$tot7c = $tot7c + $num;
		$data[] = $num;
		$num = intval(quickQuery("SELECT COUNT(*) as result FROM ".sql_table('item')." WHERE iblog=".intval($row['bid'])." AND UNIX_TIMESTAMP(itime) > UNIX_TIMESTAMP() - 2592000 "));
		$tot30 = $tot30 + $num;
		$data[] = $num;
		...
		$tot30c = $tot30c + $num;
		$data[] = $num;
		$num = intval(quickQuery("SELECT COUNT(*) as result FROM ".sql_table('item')." WHERE iblog=".intval($row['bid'])." AND UNIX_TIMESTAMP(itime) > UNIX_TIMESTAMP() - 31536000 "));
		$tot365 = $tot365 + $num;
		$data[] = $num;
		...
		$tot365c = $tot365c + $num;
		$data[] = $num;
		$num = intval(quickQuery("SELECT COUNT(*) as result FROM ".sql_table('item')." WHERE iblog=".intval($row['bid']).""));
		$tot = $tot + $num;
		$data[] = $num;
