...
/*
	Version history:
	- 1.4.2
        * modify skinvar to treat def_litag option as template for custom display of links
        * allowable tags are %url%, %link%, %desc%, %delete%, %verify%, %suspend%, %approve%
        * old values of def_litag give same result as before (backward compatible.
	- 1.4.1:
        * fix bug when adding a duplicate entry caused mysql error
	- 1.4.0:
...
// end of class-wide variables
	...
	function install() {
		...
		$this->createOption('DuplicateText','Duplicate Entry Text (including any html tags) to display above add form when user submits a duplicate site.','textarea','<p style="color:#ff2222;background-color:#eeeeee;">Thanks for your submission. The site you submitted is already on our list.</p>');
		...
		if ($this->getOption('del_uninstall') == 'yes')	{
			...
			sql_query("INSERT INTO ".sql_table($this->tablename.'_options')
				...
				." VALUES ('".addslashes($this->getOption('quickmenu'))."'"
							 ...
							 .", '".addslashes($this->getOption('DuplicateText'))."')"
				  );
		}
	}
	...
	function init() {
		...
		/* add option if it doesn't exist. needed to upgrade from 1.4.0 to 1.4.1 */
		if (trim($this->getOption('DuplicateText')) == '') {
			$this->createOption('DuplicateText','Duplicate Entry Text (including any html tags) to display above add form when user submits a duplicate site.','textarea','<p style="color:#ff2222;background-color:#eeeeee;">Thanks for your submission. The site you submitted is already on our list.</p>');
		}
	}
	...
	function doSkinVar($skinType, $what = 'list',$nshow = '',$sort = 'random',$litag = '',$sman = '') {
		...
		while ($site = mysql_fetch_object($sites)) {
			...
			$link = '<a href="'.htmlentities($site->url).'" ';
			if ($site->alt) $link .= 'title="'.htmlentities($site->alt).'" '; 
			$link .= '>'.$site->title.'</a>'."\n";
			
			$deletelink = '<a href="'.$CONF['ActionURL'].'?action=plugin&amp;name=SiteList&amp;type=deleteurl&amp;url='.htmlentities($site->ourl).'">['._SITELIST_DELETE."]</a>\n";
			$verifylink = '<a href="'.$CONF['ActionURL'].'?action=plugin&amp;name=SiteList&amp;type=verifyurl&amp;url='.htmlentities($site->url).'&amp;nsusp='.$site->suspended.'">['._SITELIST_VERIFY."]</a>\n";
			$suspendlink = '<a href="'.$CONF['ActionURL'].'?action=plugin&amp;name=SiteList&amp;type=suspendurl&amp;url='.htmlentities($site->ourl).'&amp;nsusp='.$site->suspended.'">['._SITELIST_SUSPEND."]</a>\n";
			$approvelink = '<a href="'.$CONF['ActionURL'].'?action=plugin&amp;name=SiteList&amp;type=checked&amp;url='.htmlentities($site->url).'">['._SITELIST_APPROVE."]</a>\n";
			
			$tagarr = array('%url%','%link%','%desc%','%delete%','%verify%','%suspend%','%approve%');
			$valarr = array($site->url,$link,$site->title,$deletelink,$verifylink,$suspendlink,$approvelink);
			
			if (strpos($litag,'%') === false) {
				echo $slitag.$link."\n";
				if ($sman == 'yes') {
					if ($admin) {
						echo " $deletelink\n";
						echo " $verifylink\n";
						if (!$site->suspended) echo " $suspendlink\n";
						if (!$site->checked) echo " $approvelink\n";
					}
				}
				echo $elitag."\n";
			}
			else {
				echo str_replace($tagarr,$valarr,$litag);
			}
		}
	}
...
// Get input variables and set some parameters
		...
		$destURL = str_replace(array("?thanks=1","&thanks=1","?thanks=2","&thanks=2","&list=1"),array("","","","","&list=0"),$destURL);
...
// determine action and perform it
		switch($type) {
			...
			case 'addurl':
				...
				if (!$this->addURL($url, $desc, $alt)) {
					if (strpos($destURL,'?') === false)
						$destURL = $destURL.'?thanks=2';
					else
						$destURL = $destURL.'&thanks=2';
					break;
				}
...
// unchecked sites, and will need admin review.
	...
	function showAddForm($nshow = '',$siteurl = 'http://',$sitedesc = 'Site Description',$type = 'addurl') {
        ...
        if ($this->getOption('members_only') == 'yes' && !$member->isLoggedIn()) {
			...
			if (requestVar('thanks') == '2')
                echo $this->getOption('DuplicateText');
            ?>
	...
	if ($this->_urlExists($url)) return 0;
	...
	function modURL(&$url, $desc, $ourl, $odesc, $alt, $oalt) {
		...
		if (!$this->_urlExists($purl)) return $success;
...
// if basic string (no preg format) make into case insensitve search
	...
	*/
	...
	/* function to see if url already exists. 
	Should use this after prepping the URL, and before running the db queriers to add or modify the db.
	*/
	function _urlExists($url) {
		return intval(quickQuery("SELECT COUNT(*) as result FROM ".sql_table($this->tablename)." WHERE url='".addslashes($url)."'"));		
	}
}
?>
