...
/** 
  ...
  * v0.6.6
  *  - fixed bug: now sendind conten-type and encoding even when sending update
  * v0.6.7
  *  - fixed bug: forgot to test if smileys conversion is turned on or not.
  * v0.6.8
  *  - [FIX] fixed buf with page navigation link asfter update.
  * v0.6.9
  *  - FIX: fixed php5 reference issue
  *
*/
...
class NP_MiniForum extends NucleusPlugin {
 ...
 function getVersion() { return '0.6.9'; }
 ...
 */
 function showPosts($forumId,$templName,$postsPerPage) {
    ...
    while ($post = sql_fetch_array($result)) {
		...
		$this->wrapLongWords($body,$this->getOption('MFMaxLineLength'));
		$body = $templ->emoToImg ? $body = $this->insertEmoticons($body)  : $body ; 
