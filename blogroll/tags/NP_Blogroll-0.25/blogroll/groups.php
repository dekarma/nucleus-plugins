<?php
  function _addGroup($owner, $name, $desc) {
    ...
    if ($result['id'] != "") echo "<p>A group by that name already exists.</p>";
      ...
      $query = sql_query("INSERT INTO `".sql_table('plug_blogroll_groups')."` VALUES (\"\",\"$owner\",\"$name\",\"$desc\")");
  ...
  if ($_POST['action'] != "") {
    switch ($_POST['action']) {
      ...
      case "delgroup": _delGroup($_POST['id']); _makeGroupForm("add", $_GET['id']); _listGroups($memberid); break;
      case "editgroup": _editGroup($_POST['id'], $_POST['name'], $_POST['desc']); _listGroups($memberid); _makeGroupForm("add", $_GET['id']); break;
    }
  }
