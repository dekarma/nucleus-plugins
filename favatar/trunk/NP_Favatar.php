 ...
 * Changelog:
 * 1.2
 *    [ADD] Detect certain shared bloghosts and use a common favicon (eg blogspot.com, wordpress.com)
 *    [FIX] When installing, plugin will only populate favicon data for the 100 most recent comments. This is to reduce install time for Nucleus sites with a large number of comments.
 * 1.1    24-01-2006
 *    [FIX] plugin updates entry in the SQL table instead of trying to reinsert it
 *    [ADD] getTableList() function added so plugin's SQL table is included in Nucleus backups
 * 1.0		21-10-2005
 *		Initial release
 */
 ...
 class NP_Favatar extends NucleusPlugin {
  ...
  function install() {
    ...
    $query = 'SELECT DISTINCT `cmail` FROM `'.sql_table('comment').'` WHERE `cmail` != "" AND `cmail` != "http://" ORDER BY `ctime` DESC LIMIT 0,100';
    $result = sql_query($query);
    while ($row = mysql_fetch_assoc($result)) {
  ...
  function _insertURL($url) {
    $faviconURL = $this->_getCommonFavicon($url); //See if site is part of a shared bloghosting account
    if ($faviconURL != false) return $faviconURL;    
    $url = rtrim($url,'/');
    $result = sql_query('SELECT `icon`,UNIX_TIMESTAMP(`modified`) AS `modified` FROM `'.sql_table('plug_favatar').'` WHERE `url` = "'.$url.'"');
    if (mysql_num_rows($result) == 0) { //If favicon URL not found in db, insert it now
      ...
      if ($now - $row['modified'] > $offset) { //If icon URL has not been refreshed recently (set by plugin options), refresh it now
        $icon = $this->_getFavicon($url);
  ...
  function _getCommonFavicon ($link) {
    $url_parts = parse_url($link);
    if (empty($url_parts['host'])) return false;
    
    //List of common URLs
    $favicons = array (
      'blogspot.com' => 'http://blogger.com/favicon.ico',
      'wordpress.com' =>'http://wordpress.com/favicon.ico',
      'typepad.com' => 'http://www.typepad.com/favicon.ico',
      'livejournal.com' => 'http://www.livejournal.com/favicon.ico',
      'xanga.com' => 'http://www.xanga.com/favicon.ico',
      '360.yahoo.com' => 'http://360.yahoo.com/favicon.ico',
      'spaces.msn.com' => 'http://spaces.msn.com/favicon.ico',
      'blogeasy.com' => 'http://www.blogeasy.com/favicon.ico',
      'blogsome.com' => 'http://www.blogsome.com/favicon.ico'
    );
    
    foreach ($favicons as $host => $icon) {
      if (ereg($host.'$',$url_parts['host'])) return $icon;
    }
