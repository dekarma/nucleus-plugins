<?php
    ...
    * History:
    *
    * v 1.37 - improvement to thumbnail caching
    *
    ...
    *
    * v 1.34.01 - fix XHTML 1.0 compliance issues (thanks sawo)
    * 
	...
	function parse($matches) {
        ...
        foreach ($images as $image) {
            ...
            if ((strpos(strtolower($image),strtolower($this->getOption('imageURL'))) !== false) || (strpos($image,"http://") !== false)) {
                if ($k > $numtoshow) {
					$r .= '<a href="'.$image.'" rel="'.$reltext.'" title="'.$caption.'" style="display:none">';
					$useThumbs = 'no';
				}
				else {
                ...
                if ($useThumbs == 'yes') {
                    $r .='<img src="'.$this->getAdminURL().'thumbnail.php?image='.$image.'&amp;size='.$this->getOption('maxSize').'" alt="'.$caption.'" style="border:0px solid" />';
                } else {
                    $r .= $caption;
                }
                $r .= '</a>';
