 ...
 *************************************/
	if ($showlist == 'cats') {
		...
		echo "<table>\n";
		echo "<tr class=\"h\">\n";
		echo "<th>Category ID</th><th>Name</th><th>Desc</th><th>Order</th><th>Template</th><th>Show on Main Page?</th><th></th></tr>\n";
		$ires = sql_query("SELECT c.catid as catid, o.onumber as myorder, c.cdesc as cdesc, c.cname as catname, o.otemplate as template, o.omainpage as mainpage FROM ".sql_table('category')." as c, ".sql_table('plug_ordered_cat')." as o WHERE c.catid = o.ocatid AND c.cblog = $bshow AND o.onumber>0 ORDER BY o.onumber ASC");
		while ($items = mysql_fetch_assoc($ires)) {
			oceditform($items,$bshow);
		}
		$ires = sql_query("SELECT c.catid as catid, o.onumber as myorder, c.cdesc as cdesc, c.cname as catname, o.otemplate as template, o.omainpage as mainpage FROM ".sql_table('category')." as c, ".sql_table('plug_ordered_cat')." as o WHERE c.catid = o.ocatid AND c.cblog = $bshow AND o.onumber=0 ORDER BY c.cname ASC");
		while ($items = mysql_fetch_assoc($ires)) {
			oceditform($items,$bshow);
		}
