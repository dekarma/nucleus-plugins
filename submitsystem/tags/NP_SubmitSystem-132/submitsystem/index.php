...
if (isset($_GET['allow']) || isset($_GET['deny'])) {
	...
	if ($id > 0) {
		...
		if (is_array($array) && (($ss->getOption('moderatormode') == 'team' && $member->isTeamMember($row['ss_blogid'])) || ($ss->getOption('moderatormode') == 'teamadmin' && $member->isBlogAdmin($row['ss_blogid'])) || $member->isAdmin())) {
...
if ($_GET['page'] == 'log' && $ss->getOption('log') == 'yes' && $member->isAdmin()) {
	echo('<h3>IP log</h3>');
...
elseif ($_GET['page'] == 'showbody' && is_numeric($_GET['id']) && $_GET['id'] > 0) {
	$result = sql_query('SELECT * FROM ' . $ss->dbtable . ' WHERE ss_id = ' . $_GET['id']);
	while($row = mysql_fetch_array($result)) {
		echo('<h3>' . $row['ss_title'] . '</h3>');
		echo('<p>' . $row['ss_body'] . '</p>');
	}
}
else {
	...
	echo('<table>');
	echo('<thead><tr><td><b>Blog</b></td><td><b>Title</b></td><td><b>Body</b></td><td><b>Poster</b></td><td><b>Date</b></td><td><b>Extra fields</b></td><td><b>Files</b></td><td><b>IP</b></td><td><b>Actions</b></td></tr></thead>');
	...
	while($row = mysql_fetch_array($result)) {
		if (($ss->getOption('moderatormode') == 'team' && $member->isTeamMember($row['ss_blogid'])) || ($ss->getOption('moderatormode') == 'teamadmin' && $member->isBlogAdmin($row['ss_blogid'])) || $member->isAdmin()) {
			...
			echo('<td><a href="' . $_SERVER['PHP_SELF'] . '?page=showbody&id=' . $row['ss_id'] . '">show</a></td>');
			...
			if (!empty($row['ss_files'])) {
				...
				foreach (explode('|', $row['ss_files']) as $file) {
					...
					echo('<a href="../../../media/' . $file . '">' . $file . '</a>');
					$first = false;
				}
			}
			echo('</td>');
			echo('<td>' . $row['ss_poster_ip'] . '</td>');
			echo('<td><a href="' . $_SERVER['PHP_SELF'] . '?allow=' . $row['ss_id'] . '">Allow</a> | <a href="' . $_SERVER['PHP_SELF'] . '?deny=' . $row['ss_id'] . '">Deny</a></td>');
			echo('</tr>');
		}
	}
	echo('</table>');
