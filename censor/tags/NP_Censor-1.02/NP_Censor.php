<?php
	...
	*/
     ...
     * 1.02 (07/09/2007) - fix bug when have blank banned word
	 *
	 */
...
class NP_Censor extends NucleusPlugin
{
	...
	function event_PreAddComment(&$data) {
		...
		if ($this->getBlogOption($bid, 'blockComments') == 'yes' || $this->getOption('force_blockComments') == 'yes') {
			...
			foreach ($bannedwords as $value) {
				$value = $this->prepBannedWord($value);
				if ($value == '' || strpos(strtolower($data['comment']['body']), strtolower($value)) === false) {}
				else {
                    $nbw += 1;
	...
	function getBannedWords($bid) {
        $sitebans = $this->getOption('def_bannedWords');
        $blogbans = $this->getBlogOption($bid,'bannedWords');
        if ($sitebans == '' || $blogbans == '') $haveboth = '';
        else $haveboth = ',';
		$badwords = $sitebans.$haveboth.$blogbans;
